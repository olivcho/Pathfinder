...
function runTests() {
    ...
    const assistant = new BRTResearchAssistant();

    // Test constructor initialization
    TestRunner.run('Constructor initializes properly', () => {
        TestRunner.assertTrue(assistant.API_KEY !== undefined, 'API key should be defined');
        TestRunner.assertTrue(assistant.systemPrompt !== undefined, 'System prompt should be defined');
        TestRunner.assertTrue(Array.isArray(assistant.history), 'History should be an array');
    });

    // Test message adding
    TestRunner.run('Add message works correctly', () => {
        assistant.addMessage('Test message', 'user');
        const messages = assistant.chatMessages.getElementsByClassName('user-message');
        TestRunner.assertEquals(messages.length, 1, 'Should add one message');
        TestRunner.assertTrue(messages[0].textContent.includes('Test message'), 'Message content should match');
    });

    // Test history management
    TestRunner.run('History management works correctly', () => {
        assistant.updateHistory('User message', 'Bot response');
        TestRunner.assertEquals(assistant.history.length, 2, 'History should contain user and bot message');
        TestRunner.assertEquals(assistant.history[0].role, 'user', 'First message should be from user');
        TestRunner.assertEquals(assistant.history[1].role, 'model', 'Second message should be from model');
    });

    // Test conversation saving
    TestRunner.run('Conversation saving works correctly', () => {
        assistant.saveConversation();
        const saved = localStorage.getItem('brtConversation');
        TestRunner.assertTrue(saved !== null, 'Conversation should be saved to localStorage');
        const parsed = JSON.parse(saved);
        TestRunner.assertTrue(Array.isArray(parsed.history), 'Saved history should be an array');
    });

    // Test error handling
    TestRunner.run('Error handling works correctly', () => {
        assistant.showError('Test error');
        const errorMessages = assistant.chatMessages.getElementsByClassName('error-message');
        TestRunner.assertEquals(errorMessages.length, 1, 'Should show one error message');
        TestRunner.assertEquals(errorMessages[0].textContent, 'Test error', 'Error message content should match');
    });

    // Test loading state
    TestRunner.run('Loading state works correctly', () => {
        assistant.setLoading(true);
        TestRunner.assertTrue(assistant.sendButton.disabled, 'Send button should be disabled while loading');
        TestRunner.assertTrue(assistant.userInput.disabled, 'Input should be disabled while loading');
        TestRunner.assertEquals(assistant.sendButton.textContent, 'Sending...', 'Button text should indicate loading');
        
        assistant.setLoading(false);
        TestRunner.assertTrue(!assistant.sendButton.disabled, 'Send button should be enabled after loading');
        TestRunner.assertTrue(!assistant.userInput.disabled, 'Input should be enabled after loading');
        TestRunner.assertEquals(assistant.sendButton.textContent, 'Send', 'Button text should return to normal');
    });

    console.log('All tests completed!');
}

// Run tests only if in test environment
